...
describe('Conversation', () => {
  ...
  describe('Serialization (toJSON / restore)', () => {
    it('round-trips messages and timeline', () => {
      conversation.addUserMessage('hello');
      conversation.handleSessionUpdate({
        sessionUpdate: 'agent_message_chunk',
      ...
      expect(restored.timeline).toEqual(conversation.timeline);
    });
    ...
    it('round-trips tool calls', () => {
      ...
      expect(restored.toolCalls.get('tc1')?.title).toBe('Read file');
      expect(restored.timeline).toEqual([{ type: 'toolCall', toolCallId: 'tc1' }]);
    });

    it('round-trips permissions', () => {
      conversation.trackPermission(1, 'tc1', 'Allow read?', [
        { optionId: 'yes', name: 'Yes', kind: 'allow_once' },
      ]);
      ...
      expect(restored.permissions).toHaveLength(1);
      expect(restored.permissions[0].title).toBe('Allow read?');
    });

    it('round-trips shell results', () => {
      conversation.addShellResult('ls', 'file.txt\n', '', 0);
      conversation.addShellResult('pwd', '/home\n', '', 0);

      const snapshot = conversation.toJSON();
      const restored = new Conversation();
      restored.restore(snapshot);

      expect(restored.shellResults.size).toBe(2);
      expect(restored.shellResults.get(0)?.command).toBe('ls');
      expect(restored.shellResults.get(1)?.command).toBe('pwd');
    });

    it('round-trips plan', () => {
      conversation.handleSessionUpdate({
        sessionUpdate: 'plan',
        entries: [{ title: 'Step 1', status: 'pending', priority: 1 }],
      });
      ...
      expect(restored.plan?.entries).toHaveLength(1);
      expect(restored.plan?.entries[0].title).toBe('Step 1');
    });

    it('survives JSON.stringify/parse (simulates sessionStorage)', () => {
      conversation.addUserMessage('test');
      conversation.handleSessionUpdate({
        sessionUpdate: 'agent_message_chunk',
        content: { type: 'text', text: 'response' },
      });
      conversation.addShellResult('echo hi', 'hi\n', '', 0);
      ...
      expect(restored.messages).toEqual(conversation.messages);
      expect(restored.shellResults.size).toBe(1);
      expect(restored.timeline).toEqual(conversation.timeline);
    });

    it('notifies listeners after restore', () => {
      const listener = vi.fn();
      const target = new Conversation();
      target.onChange(listener);

      conversation.addUserMessage('hi');
      target.restore(conversation.toJSON());

      expect(listener).toHaveBeenCalled();
    });

    it('avoids thinking ID collisions after restore', () => {
      // Create thinking entries
      conversation.handleSessionUpdate({
        sessionUpdate: 'agent_thought_chunk',
      ...
      // New thinking after restore should use thinking-1, not thinking-0
      restored.handleSessionUpdate({
        sessionUpdate: 'agent_thought_chunk',
        content: { type: 'text', text: 'More thinking...' },
      });

      expect(restored.toolCalls.has('thinking-0')).toBe(true);
      expect(restored.toolCalls.has('thinking-1')).toBe(true);
      // Original thinking-0 should not be overwritten
      expect(restored.toolCalls.get('thinking-0')!.content[0]).toMatchObject({
        type: 'content',
        content: { type: 'text', text: 'Thinking...' },
      });
    });
